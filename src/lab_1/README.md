...
#### `_name` – имя пациента (строка, непустая)

#### `_age` – возраст (целое число, 0–130)

#### `_diagnosis` – диагноз (строка, может быть пустой)

#### `_record_number` – уникальный номер медицинской карты (строка или число)

#### `_is_active` – состояние пациента (активен / выписан, булево)
...
### `_validate_record_number(record_number)`
